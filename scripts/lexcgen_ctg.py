...
parser.add_argument("--write_file", type=str, required=True, help="LexC-Gen generated file")
parser.add_argument("--total", type=int, required=True, help="number of generated examples")
...
parser.add_argument("--notes", type=str, default='', help="notes to include in output file name")
parser.add_argument("--top_p", type=float, default=0.1, help="(best) top p")
parser.add_argument("--temperature", type=float, default=1.0, help="(best) temperature")
...
model = AutoModelForCausalLM.from_pretrained(
        ...
        cache_dir="/users/zyong2/data/zyong2/huggingface" # TODO: remove for anonymity
)
...
write_file = pathlib.Path(args.write_file)
write_file.parent.mkdir(parents=True, exist_ok=True)

wf = open(write_file, "w+")
total = args.total

print("Generating", total, "examples")
for cur_idx in tqdm(range(total)):
    tokens = random.choices(list(lexicons.keys()), k=10)  # choose 10 tokens
    ...
    output_txt = tokenizer.decode(output[0], skip_special_tokens=False)  # TODO: can be optimized with batch generation
    wf.write(output_txt)
    wf.write("\n")
