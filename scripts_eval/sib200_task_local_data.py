"""
Example usage:
python3 ./eval_scripts/sib_task_hf_data.py \
	--lang "ak" \
	--lexcgen_size "100k" \
	--num_epochs 100
"""
...
train_dataset_df = pd.DataFrame(pd.read_csv(args.train_csv_path, sep="\t"))
train_dataset = Dataset.from_pandas(train_dataset_df)

val_dataset_df = pd.DataFrame(pd.read_csv(args.valid_csv_path, sep="\t"))
val_dataset = Dataset.from_pandas(val_dataset_df)

test_dataset_df = pd.DataFrame(pd.read_csv(args.test_csv_path, sep="\t"))
...
############# Train Model #############
output_dir = pathlib.Path(args.output_dir) / f"eval-sib200-{args.target_lang}-{args.model_name.split('/')[-1]}"
